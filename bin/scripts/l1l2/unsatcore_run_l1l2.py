...
def execute_command_and_get_data(file_path):
    kind2_command = f"/home/ubuntu/Lustre-exp/bin/exe/U-IC3_l1l2 --enable IC3QE  -vv --timeout 290 --color false  {file_path}"
    print(kind2_command)
...
def main():
    ...
    output_file = '/home/ubuntu/Lustre-exp/bin/result/l1l2/U-IC3_l1l2.csv'  # csv_file
    ...
    with open(output_file, 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["Unsatcore_file_path", "Unsatcore_time", "Unsatcore_k", "Unsatcore_solver", "Unsatcore_neg_state", "Unsatcore_status", 
                        "Unsatcore_frame_sizes", "Unsatcore_elapsed_time"])  # 修正了标题行的格式
