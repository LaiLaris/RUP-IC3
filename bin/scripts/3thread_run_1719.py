...
def execute_command_and_get_data(file_path):
    kind2_command = f"/home/ubuntu/Lustre-exp/bin/exe/P-IC3 --enable IC3QE --enable IC3QEl1l2 --enable IC3QEl2l1 -vv --timeout 290 --color false  {file_path}"
    print(kind2_command)
    ...
    try:
        ...
        invalid_match = re.search(r'<Failure> Property .* is invalid .*', output)
         ...
         # 如果匹配到valid或invalid，检查后面的sorting clause
        sorting_clause_match = None
        if valid_match or invalid_match:
            sorting_clause_match = re.search(r'(?:<Success> Property .+|<Failure> Property .+)\s+sorting clause\s+(l2l1|l1l2)', output)

        
        # 如果没有找到排序方式，返回 'base'
        if not sorting_clause_match:
            sorting_clause_match = 'base'
        else:
            sorting_clause_match = sorting_clause_match.group(1)
        ...
        # 返回结果
        return sorting_clause_match,total_time, k, solver, neg_state_solver, status, frame_sizes, 

    except subprocess.TimeoutExpired:
        return "timeout", "N/A", "N/A", "N/A", "N/A", "N/A", 'N/A','N/A'

# 写入输出文件
def append_result_to_csv(file_path, sorting_clause_match, total_time, k, solver, neg_state_solver, status, 
                         frame_sizes, elapsed_time, output_file):
    with open(output_file, 'a', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow([file_path, sorting_clause_match,total_time, k, solver, neg_state_solver, status, 
                         frame_sizes, f"{elapsed_time:.3f}"])  # 移除total_elapsed_time

def process_file(file_path, output_file):
    elapsed_start_time = time.time()  # 记录每个文件处理的开始时间
    
    sorting_clause_match, total_time, k, solver, neg_state_solver, status, frame_sizes= execute_command_and_get_data(file_path)
    ...
    # 写入 CSV 文件
    append_result_to_csv(file_path, sorting_clause_match, total_time, k, solver, neg_state_solver, status, 
                          frame_sizes, elapsed_time, output_file)
...
def main():
    ...
    input_file = '/home/ubuntu/Lustre-exp/bin/path/1719_path.txt'  # lus_path
    output_file = '/home/ubuntu/Lustre-exp/bin/result/P-IC3_1719.csv'  # csv_file
    ...
    with open(output_file, 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["3Thread_file_path", "3Thread_sorting_clause", "3Thread_time", "3Thread_k", "3Thread_solver", "3Thread_neg_state", "3Thread_status", 
                        "3Thread_frame_sizes", "3Thread_elapsed_time"])  # 修正了标题行的格式
