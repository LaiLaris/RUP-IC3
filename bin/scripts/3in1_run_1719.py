...
def execute_command_and_get_data(file_path):
    ...
    kind2_command = f"/home/ubuntu/Lustre-exp/bin/exe/RUP-IC3 --enable IC3QE --enable IC3QEl1l2 --enable IC3QEl2l1 -vv --timeout 490 --color false  {file_path}"
    print(kind2_command)
    args = shlex.split(kind2_command)

    
    try:
        result = subprocess.run(args, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=500)
        ...
        if valid_match:
            ...
            status = "timeout"
        return sorting_clause_match, total_time, k, solver, neg_state_solver, reuse_num_s, reuse_num_t, reuse_num_gen,status, frame_sizes
    except subprocess.TimeoutExpired:
        return "timeout", "N/A", "N/A", "N/A", "N/A", "N/A", "N/A", "N/A","N/A","N/A","N/A","N/A"

# 写入输出文件
def append_result_to_csv(file_path, sorting_clause_match, total_time, k, solver, neg_state_solver, 
                         reuse_num_s, reuse_num_t, reuse_num_gen, status, frame_sizes, elapsed_time, output_file):
    with open(output_file, 'a', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow([file_path, sorting_clause_match, total_time, k, solver, neg_state_solver, 
...
def main():
    ...
    output_file = '/home/ubuntu/Lustre-exp/bin/result/RUP-IC3_1719.csv'  # csv_file
    ...
    with open(output_file, 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["3in1_file_path", "3in1_sorting_clause", "3in1_time", "3in1_k", "3in1_solver", "3in1_neg_state", "3in1_num_s", "3in1_num_t", "3in1_num_gen", "3in1_status", "3in1_frame_sizes", "3in1_elapsed_time"])
